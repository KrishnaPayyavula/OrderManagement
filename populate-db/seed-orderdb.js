...
const region = 'eu-north-1'; // It reads the env. variable
...
async function initData(dynamoDb) {
    ...
    order2.quantity = 3;
    try{
        let result= await dynamoDb.put({ Item: order2,TableName: ordersTable }).promise(); 
        console.log(result)
    }catch(err){
        console.log(err);
    }
...
async function A(dynamoDb){
    await initData(dynamoDb)
}
A(dynamoDb)
